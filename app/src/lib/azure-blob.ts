...
// Matched case-insensitively against the classifier's doc_type.
...
export function isLocalOnly(docType: string): boolean {
  ...
  return LOCAL_ONLY_DOC_TYPES.has(docType.toLowerCase().trim());
}
...
/** Upload redacted doc + manifest JSON in one call. Returns SAS download URLs. */
export async function uploadDocToAzure(opts: {
  mrn: string;
  jobId: string;
  blobName: string;          // e.g. "P0008_discharge_summary_20260614.jpg"
